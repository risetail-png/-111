...
def convert_pdf_rotated(pdf_path, page_idx, dpi):
    """
    PDFの指定ページを画像化し、/Rotate に合わせて回転して返す。
    Popplerは/Rotateを自動で反映しないことがあるため、明示的に補正する。
    戻り値: PIL.Image（表示向きに補正済み）または None
    """
    ...
    try:
        ...
        rot = int(reader.pages[page_idx].get('/Rotate', 0))
    except Exception:
        rot = 0

    # PIL の rotate は反時計回り。PDF Rotate は時計回りなので符号を反転
    if rot == 90:
...
# ── 損傷図の写真番号パターン（N-M 形式） ─────────────────────────────────────
# 「写真番号1-1」「写真番号1-2(1-1)」など
RE_DIAG_PHOTO_NUM = re.compile(
    r'写真番号\s*(\d+)\s*[-－]\s*(\d+)'   # 径間番号-写真番号
)
...
def _parse_diag_photo_nums(text):
    """
    損傷図ページOCRテキストから写真番号セットを返す。
    対応形式:
      - 「写真番号1-3」「写真番号1-2(1-1)」→ N-M形式: M を採用
      - 「写真番号5,6 7,8」「写真番号5 6」  → カンマ/スペース区切り形式
      - 「写真番号5」                        → 単独形式
    括弧内（前回番号）は無視する。
    """
    text = _normalize_text(text)
    text = re.sub(r'前回\s*[-－]?\s*\d*', '', text)   # 「前回-N」除去
    results = set()

    # N-M形式を処理しつつ、該当部分をテキストから除去する
    # （除去しないと径間番号側の数字も単独形式で拾ってしまう）
    def _collect_nm(m):
    ...
    # 残ったテキストから単独・カンマ・スペース区切り形式を収集
    # 「写真番号5,6 7,8」「写真番号5 6 7 8」「写真番号21」など
    for m in re.finditer(r'写真番号\s*([\d][,，、・\s\d]*)', text2):
        chunk = m.group(1)
        if re.search(r'[-－]', chunk):
            continue  # N-M形式の残りは除外
...
def ocr_diagram_page(pdf_path, page_idx, dpi):
    """
    損傷図ページをRotateを考慮してOCRし、写真番号セットを返す。
    """
    img = convert_pdf_rotated(pdf_path, page_idx, dpi)
    if img is None:
        return set()
    t = pytesseract.image_to_string(img, lang='jpn', config='--psm 6')
...
def _parse_photo_page_nums(text):
    ...
    work = re.sub(r'前回写真\s*\d+\s*[-－]\s*\d+', '', work)  # 「前回写真N-M」除去
    work = re.sub(r'前回\s*[-－]?\s*\d*', '', work)          # 「前回-N」「前回N」「前回」
    work = re.sub(r'[-－]\s*\d+', '', work)                  # 残った「-N」
    work = re.sub(r'起点側\s*終点側\s*\d+', '', work)         # 「起点側 終点側N」（径間番号ヘッダー）除去
    ...
    # 撮影日除去後のテキスト全体から数字を拾う
    # 「写真番号1 2 3\n4\n...2020... 5 6」のように撮影日行の後ろに紛れる番号も対象
    for m in re.finditer(r'\b(\d{1,3})\b', work):
        n = int(m.group(1))
        if base <= n <= upper:
            nums.append(n)
...
def add_buttons_to_page(pdf, page_idx, btn_list, page_w, page_h,
    ...
    """
    ページにボタンを追加する。
    page_w, page_h は get_page_size() が返す「表示上の」幅・高さ。
    Rotate を考慮して PDF 座標系に変換して配置する。
    ボタンは表示上の「下辺」に横並びで配置する。
    """
    page = pdf.pages[page_idx]
    mb   = page['/MediaBox']
    raw_w, raw_h = float(mb[2]), float(mb[3])
    rot  = int(page.get('/Rotate', 0))

    # ── 表示上の座標でボタン領域を定義 ──────────────────────────────────────
    # 表示上: 左マージン=64pt, 右マージン=48pt, 下から BTN_Y1〜BTN_Y2
    margin_l  = 64.0
    ...
    # ── Rotate に応じて PDF 座標系の cm 行列を計算 ───────────────────────────
    # 表示上の矩形: (margin_l, BTN_Y1) 〜 (margin_r, BTN_Y2)  ※左下原点
    # PDF座標系は Rotate 適用前なので逆変換する
    if rot == 0:
        # そのまま
        cm = (f"{btn_total:.4f} 0 0 {BTN_H:.4f} "
              f"{margin_l:.4f} {BTN_Y1:.4f}")
        # アノテーション矩形（PDF座標）
        ann_x1 = margin_l
        ann_y1 = BTN_Y1
        ann_x2 = margin_r
        ...
        # 表示上(x,y) → PDF座標(y, raw_w - x)  ※raw_w=MediaBox幅
        # ボタン左下(表示上): (margin_l, BTN_Y1)
        # → PDF座標: (BTN_Y1, raw_w - margin_l)
        # ボタン画像は表示上で水平なので、PDF座標系では垂直に配置
        # cm: [a b c d e f]  画像(0,0)〜(1,1) → PDF座標矩形
        # 表示上の幅=btn_total → PDF座標でのy方向の長さ
        # 表示上の高さ=BTN_H   → PDF座標でのx方向の長さ
        pdf_x0 = BTN_Y1                    # PDF左下x
        pdf_y0 = raw_w - margin_r          # PDF左下y
        cm = (f"0 {btn_total:.4f} -{BTN_H:.4f} 0 "
              f"{pdf_x0:.4f} {pdf_y0:.4f}")
        ann_x1 = BTN_Y1
        ann_y1 = raw_w - margin_r
        ann_x2 = BTN_Y2
        ann_y2 = raw_w - margin_l
    elif rot == 270:
        pdf_x0 = raw_h - BTN_Y2
        pdf_y0 = margin_l
        cm = (f"0 -{btn_total:.4f} {BTN_H:.4f} 0 "
        ...
        pdf_x0 = raw_w - margin_r
        pdf_y0 = raw_h - BTN_Y2
        cm = (f"-{btn_total:.4f} 0 0 -{BTN_H:.4f} "
              f"{raw_w - margin_l:.4f} {raw_h - BTN_Y1:.4f}")
        ann_x1 = raw_w - margin_r
        ann_y1 = raw_h - BTN_Y2
        ann_x2 = raw_w - margin_l
        ann_y2 = raw_h - BTN_Y1
    else:
        # 未対応のRotateはrot=0と同様に扱う
        cm = (f"{btn_total:.4f} 0 0 {BTN_H:.4f} "
              f"{margin_l:.4f} {BTN_Y1:.4f}")
        ann_x1 = margin_l
        ann_y1 = BTN_Y1
    ...
    for i, (_, target_idx) in enumerate(btn_list):
        # 表示上のボタン個別x範囲
        disp_x1 = margin_l + BTN_GAP + i * (btn_w_pt + BTN_GAP)
        disp_x2 = disp_x1 + btn_w_pt

        if rot == 0:
            rx1, ry1, rx2, ry2 = disp_x1, BTN_Y1, disp_x2, BTN_Y2
        elif rot == 90:
            rx1 = BTN_Y1
            ry1 = raw_w - disp_x2
            rx2 = BTN_Y2
        ...
        annots.append(pdf.make_indirect(Dictionary(
            ...
            Rect=Array([pikepdf.Real(rx1), pikepdf.Real(ry1),
                        pikepdf.Real(rx2), pikepdf.Real(ry2)]),
...
def run_process(input_path, output_path, dpi, log_cb, done_cb):
    ...
    try:
        ...
        # ── 損傷写真ページの径間番号・写真番号を取得 ────────────────────────
        log_cb("損傷写真ページの写真番号を取得中...")
        photo_page_nums  = {}   # page_idx -> [nums]
        photo_page_span  = {}   # page_idx -> span (int or None)

        for pidx in photo_pages:
            nums = get_photo_nums_on_page(input_path, pidx, dpi, use_ocr=True)
            photo_page_nums[pidx] = nums
            # 径間番号をテキスト抽出で取得（「起点側 終点側N」パターン）
            text = reader.pages[pidx].extract_text() or ""
            sm = re.search(r'起点側\s*終点側\s*(\d+)', text)
            span = int(sm.group(1)) if sm else None
            photo_page_span[pidx] = span
            log_cb(f"  p.{pidx+1}: 径間={span} 写真番号 {nums}")

        # ── 損傷写真ページを径間番号でグループ化 ───────────────────────────
        # 径間番号が直接取れる場合はそれを優先。取れない場合は連続性で推定。
        span_to_photo_pages = defaultdict(list)   # span -> [page_idx]
        unknown_span_pages  = []                  # 径間番号不明ページ

        for pidx in photo_pages:
            span = photo_page_span.get(pidx)
            if span is not None:
                span_to_photo_pages[span].append(pidx)
            else:
                unknown_span_pages.append(pidx)

        log_cb(f"損傷写真径間グループ: {dict((k, [p+1 for p in v]) for k,v in sorted(span_to_photo_pages.items()))}")

        # ── 損傷図ページの写真番号取得（テキスト抽出 → OCR補完） ────────────
        log_cb("損傷図ページの写真番号を取得中（OCR）...")
        diagram_info = {}   # page_idx -> {'span': int or None, 'nums': set of int}

        for didx in diag_pages:
            span_nums  = {}    # span -> set of photo_num  (N-M形式)
            plain_nums = set() # 単独形式
            ...
            for m in RE_DIAG_PHOTO_NUM.finditer(text_n):
                ...
                span_nums.setdefault(s, set()).add(n)
            for m in re.finditer(r'写真番号\s*(\d+)(?!\s*[-－])', text_n):
                plain_nums.add(int(m.group(1)))

            # 損傷図自身の径間番号（ヘッダー）
            diag_sm = re.search(r'起点側\s*終点側\s*(\d+)', text)
            diag_span = int(diag_sm.group(1)) if diag_sm else None

            # 2) OCR補完（テキスト抽出で写真番号が取れなかった場合）
            if not span_nums and not plain_nums:
                log_cb(f"  損傷図 p.{didx+1}: テキスト抽出で取れず、OCRを実行...")
                ocr_nums = ocr_diagram_page(input_path, didx, dpi)
                # OCR結果は写真番号のみ。損傷図自身の径間番号をspanとして付与。
                if diag_span is not None:
                    span_nums[diag_span] = ocr_nums
                else:
                    plain_nums = ocr_nums
            ...
            all_nums = set().union(*span_nums.values()) | plain_nums if span_nums else plain_nums
            log_cb(f"  p.{didx+1}: 径間={diag_span} 写真番号 {sorted(all_nums)}")

        # ── 対応関係を構築 ───────────────────────────────────────────────────
        # 損傷図の径間番号 → 損傷写真ページ（同じ径間番号を持つページ）
        diag_to_photo_pages = defaultdict(list)

        for didx, info in diagram_info.items():
            span_nums  = info['span_nums']
            plain_nums = info['plain_nums']
            diag_span  = info['diag_span']
            seen_pages = set()

            if span_nums:
                # N-M形式: 径間番号でダイレクトに損傷写真ページを特定
                for span, nums in sorted(span_nums.items()):
                    candidates = span_to_photo_pages.get(span, [])
                    for pp in candidates:
                        pg_nums = set(photo_page_nums.get(pp, []))
                        if pg_nums & nums and pp not in seen_pages:
                            diag_to_photo_pages[didx].append(pp)
                            seen_pages.add(pp)
            elif plain_nums and diag_span is not None:
                # OCR結果(plain_nums) + 損傷図自身の径間番号で照合
                candidates = span_to_photo_pages.get(diag_span, [])
                for pp in candidates:
                    pg_nums = set(photo_page_nums.get(pp, []))
                    if pg_nums & plain_nums and pp not in seen_pages:
                        diag_to_photo_pages[didx].append(pp)
                        seen_pages.add(pp)
                # 写真番号が全く一致しなければ径間一致だけで追加
                if not diag_to_photo_pages[didx]:
                    for pp in candidates:
                        if pp not in seen_pages:
                            diag_to_photo_pages[didx].append(pp)
                            seen_pages.add(pp)
            else:
                # フォールバック: 写真番号の一致で全グループを検索
                for num in sorted(plain_nums):
                    for span, ppages in span_to_photo_pages.items():
                        for pp in ppages:
                            pg_nums = set(photo_page_nums.get(pp, []))
                            if num in pg_nums and pp not in seen_pages:
                                diag_to_photo_pages[didx].append(pp)
                                seen_pages.add(pp)
        ...
        for didx, plist in diag_to_photo_pages.items():
            ...
            for pp in plist:
                ...
                label = (f"→ 写真番号 {min(photo_nums)}〜{max(photo_nums)}"
                         if photo_nums else f"→ 損傷写真 p.{pp+1}")
        ...
        for pp, dlist in photo_to_diag_pages.items():
            ...
            for didx in dlist:
                ...
                btn_list.append((f"← 損傷図（{title}）", didx))
